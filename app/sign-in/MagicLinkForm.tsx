...
export function MagicLinkForm() {
  ...
  return <main className="auth-page"><section className="auth-card"><p className="eyebrow">Leadership 360</p><h1>Access your private report</h1><p>Enter the email address where you received your report invitation. We will send a one-time sign-in link.</p><form onSubmit={submit}><label>Email address<input name="email" type="email" autoComplete="email" required /></label><button className="primary-button" type="submit" disabled={status === "sending"}>{status === "sending" ? "Sending link…" : "Email my sign-in link"}</button></form>{message ? <p className={`auth-message auth-message-${status}`} role="status">{message}</p> : null}</section></main>;
}
